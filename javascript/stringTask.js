...
to do the following: in the given string, consisting of uppercase
...
ones.
Vowels are letters "A", "O", "Y", "E", "U", "I", and the rest
are consonants. The program's input is exactly one string, it
should return the output as a single string, resulting after the
...
function stringTask(s){
    var resultStr = s.toLowerCase().split('').filter(function (element) {
        return RegExp(/a|e|i|o|u|y/).test(element) === false;
    }).join('.');
