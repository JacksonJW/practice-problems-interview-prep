...
def title_case(title, minor_words=""):
    s = set(minor_words.lower().split(" "))
    result = ""
    title_words = title.split(" ")

    for i, title_word in enumerate(title_words):
        title_word_lowercase = title_word.lower()
        title_word_capitalized = title_word.capitalize()

        if i == 0:
            result += title_word_capitalized
        elif title_word_lowercase in s:
            result += title_word_lowercase
        else:
            result += title_word_capitalized
