...
Algorithm/DS used: Modulo and iteration
...
def test_solution():
    s = Solution()
    print("Expected result from input [0,1,2] is 0 and the Actual result is: " +
          str(s.smallestEqual([0, 1, 2])))
    assert s.smallestEqual([4, 3, 2, 1]) == 2
    assert s.smallestEqual([1, 2, 3, 4, 5, 6, 7, 8, 9, 0]) == -1
    assert s.smallestEqual([2, 1, 3, 5, 2]) == 1
